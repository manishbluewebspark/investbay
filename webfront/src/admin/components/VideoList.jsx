...
//   // ✅ NEW: Function to format duration accurately
//   const formatDuration = (durationInSeconds) => {
//     const totalSeconds = Math.floor(durationInSeconds || 0);
...
//   // ✅ UPDATED: Get accurate duration from video element
...
//     // ✅ Get accurate duration if not available
//     if (!video.video_duration || video.video_duration === 0) {
//       const accurateDuration = await getVideoDuration(video.video_url);
//       if (accurateDuration) {
//         setVideos(prev => 
//           prev.map(v => v.id === video.id 
//             ? { ...v, video_duration: accurateDuration }
//             : v
//           )
//         );
//       }
...
//       {/* Rest of your modal code remains same */}
//       {activeVideo && (
//         <div
//           className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 p-4 backdrop-blur-sm"
//           onClick={() => setActiveVideo(null)}
...
//     // Pause and cleanup video when closing
...
//             <div
//               key={video.id}
//               className="relative group rounded-lg overflow-hidden transition-all duration-300 cursor-pointer hover:shadow-xl"
//               onClick={() => handleVideoClick(video)}
//               onMouseEnter={() => setHoveredVideoId(video.id)}
//               onMouseLeave={() => setHoveredVideoId(null)}
//             >
...
//                   <button
//                     onClick={(e) => openDeleteModal(video.id, e)}
//                     className="p-2 rounded-full backdrop-blur-lg bg-red-600/90 hover:bg-red-700 transition-colors shadow-lg"
//                     title="Delete video"
//                   >
//                     <FiTrash2 className="w-4 h-4 text-white" />
//                   </button>
  ...
  const handleDeleteVideo = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/videos/${id}`, {
        ...
        body: JSON.stringify({
          courseId,
          userId // This can be undefined, backend should handle it
        }),
      });
