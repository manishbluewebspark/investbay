...
//       // Filter for relevant instruments (NSE indices and stocks)
//       const relevantScrips = response.data.filter(scrip => 
//         scrip.exch_seg === 'NSE' || scrip.exch_seg === 'BSE'
//       );
      
//       // Sort alphabetically by symbol
//       const sortedScrips = relevantScrips.sort((a, b) => 
//         a.symbol.localeCompare(b.symbol)
//       );
      
//       setScripMasterData(sortedScrips);
//       toast.success(`${sortedScrips.length} instruments loaded`);
//     } catch (error) {
...
//         {"token":"99926008","symbol":"Nifty IT","name":"NIFTY IT","expiry":"","strike":"0.000000","lotsize":"1","instrumenttype":"AMXIDX","exch_seg":"NSE","tick_size":"0.000000"},
//         {"token":"99926011","symbol":"NIFTY MIDCAP 100","name":"NIFTY MIDCAP 100","expiry":"","strike":"0.000000","lotsize":"1","instrumenttype":"AMXIDX","exch_seg":"NSE","tick_size":"0.000000"}
//       ]);
//     } finally {
...
//     if (!searchQuery.trim()) {
//       return scripMasterData.slice(0, 100); // Show first 100 by default
//     }
...
//   const handleSelect = (field, value, planId = null, additionalData = null) => {
//     const updatedData = {
//       ...formData,
//       [field]: value,
//     };

//     // If selecting a script from scrip master, populate additional fields
//     if (field === 'script' && additionalData) {
//       updatedData.scriptToken = additionalData.token;
//       updatedData.scriptName = additionalData.name;
      
//       // Auto-populate exchange based on scrip data
//       if (additionalData.exch_seg) {
//         updatedData.exchange = additionalData.exch_seg;
//       }
      
//       // Auto-populate instrument type if applicable
//       if (additionalData.instrumenttype === 'AMXIDX') {
//         updatedData.instrument = 'Index';
//       } else if (additionalData.instrumenttype === 'EQ') {
//         updatedData.instrument = 'Stock';
//       }
      
//       // For options, suggest instrument type
//       if (additionalData.instrumenttype === 'OPTIDX' || additionalData.instrumenttype === 'OPTSTK') {
//         updatedData.segment = 'Options';
//       }
//     }
...
//     // If script input changes, update search query
//     if (name === 'script') {
...
//               {filteredScrips.length} instruments found • Select one
//             </div>
//           </div>
//         )}
//       </div>
...
//         </label>
//         <div
//           className={`w-full p-3 bg-white border rounded-lg cursor-pointer flex justify-between items-center transition-colors ${value ? 'border-gray-300' : 'border-gray-300 hover:border-gray-400'} ${isOpen ? 'border-gray-300' : ''} ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
...
//                     loadingPlans
//                       ? "Loading plans..."
//                       : (planOptions.length === 0 ? "No plans found" : "Select a plan")
//                   }
...
//                   <p className="mt-1 text-xs text-red-500">No subscription plans found</p>
//                 )}
//               </div>
//             </div>
...
//       setLoadingScrips(true);
//       const response = await axios.get(
//         'https://login.softwaresetu.com/api/agnelone/instrumentnew'
//       );
      
//       console.log(response.data , 10000)
...
//       toast.success(`${formattedScrips.length} instruments loaded`);
//     } catch (error) {
//       console.error('Error fetching scrip master data:', error);
//       toast.error('Failed to load instrument data');
//       // Fallback sample data with more details
//       setScripMasterData([
//         {"token":"99926000","symbol":"Nifty 50","name":"NIFTY","expiry":"","strike":"0.000000","lotsize":"1","instrumenttype":"AMXIDX","exch_seg":"NSE","tick_size":"0.000000"},
...
//   const handleScriptSelect = (scrip) => {
//     // Map Angel Broking data to your form fields
//     const mappedData = {
//       script: scrip.symbol,
//       scriptToken: scrip.token,
//       scriptName: scrip.name,
...
//     // If expiry exists in scrip data, use it
//     if (scrip.expiry && scrip.expiry.trim() !== '') {
//       mappedData.expiry = scrip.expiry;
//     }

//     // If strike price exists in scrip data, use it
...
//         </label>
//         <div
//           className={`w-full p-3 bg-white border rounded-lg flex justify-between items-center transition-colors ${value ? 'border-green-300 bg-green-50' : 'border-gray-300'} ${isOpen ? 'border-gray-300' : ''} ${loading || disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
...
//                   placeholder="Select Segment"
//                   onSelect={handleSelect}
//                   disabled={!!formData.segment} // Disable if auto-filled
//                 />
//                 <CustomDropdown
//                   label="Instrument Category"
//                   value={formData.instrument}
...
//                   placeholder="Select category"
//                   onSelect={handleSelect}
//                   disabled={!!formData.instrument} // Disable if auto-filled
//                 />
//               </div>
...
//                     onChange={handleInputChange}
//                     placeholder="30DEC2025"
//                     className={`w-full p-3 border rounded-lg focus:outline-none ${formData.expiry ? 'border-green-300 bg-green-50 text-gray-800' : 'border-gray-300 bg-white text-gray-800'}`}
//                     required
//                   />
//                 </div>
//                 <div>
...
//                     onChange={handleInputChange}
//                     placeholder="2400"
//                     className={`w-full p-3 border rounded-lg focus:outline-none ${formData.strike_price && formData.strike_price !== '' ? 'border-green-300 bg-green-50 text-gray-800' : 'border-gray-300 bg-white text-gray-800'}`}
//                     required
//                   />
//                 </div>
//               </div>
...
//                   placeholder="Select Type"
//                   onSelect={handleSelect}
//                   disabled={!!formData.instrumentType} // Disable if auto-filled
//                 />
//                 <CustomDropdown
//                   label="Trade Direction"
//                   value={formData.tradeDirection}
...
//       console.log('Scrip Master Data:', response.data);
      
//       // Add unique IDs and format data
//       const formattedScrips = response.data.data.map((scrip, index) => ({
//         ...scrip,
//         id: scrip.token || `scrip-${index}`,
//         // Add display fields for better UI
//         displaySymbol: scrip.symbol,
//         displayName: scrip.name,
//         displayExpiry: scrip.expiry || 'N/A',
//         displayStrike: scrip.strike !== '0.000000' ? parseFloat(scrip.strike).toFixed(2) : 'N/A',
//         displayLotSize: scrip.lotsize,
//         displayInstrumentType: scrip.instrumenttype,
//         displayExchange: scrip.exch_seg
//       }));
      
//       setScripMasterData(formattedScrips);
//       // toast.success(`${formattedScrips.length} instruments loaded successfully`);
//     } catch (error) {
//       console.error('Error fetching scrip master data:', error);
//       toast.error('Failed to load instrument data');
//       // Fallback sample data
...
//           "displayExpiry": "30DEC2025",
//           "displayStrike": "N/A",
//           "displayLotSize": "1",
//           "displayInstrumentType": "OPTIDX",
//           "displayExchange": "NSE"
//         },
//         {
//           "token": "12345678",
...
//       scrip.name?.toLowerCase().includes(query) ||
//       scrip.token?.includes(query) ||
//       scrip.instrumenttype?.toLowerCase().includes(query)
//     ).slice(0, 50);
//   }, [searchQuery, scripMasterData]);

//   // Map instrument types from API to your format
//   const mapInstrumentType = (apiType) => {
//     const typeMap = {
//       'EQ': 'EQ',
//       'CE': 'CE',
//       'PE': 'PE',
...
//       'FUTCUR': 'FUT'
//     };
//     return typeMap[apiType] || apiType || 'EQ';
//   };

//   // Map instrument category
//   const mapInstrumentCategory = (apiType) => {
//     const categoryMap = {
//       'EQ': 'Stock',
//       'AMXIDX': 'Index',
//       'FUTIDX': 'Future',
...
//       'PE': 'Option'
//     };
//     return categoryMap[apiType] || 'Stock';
//   };

//   // Map segment based on instrument type
//   const mapSegment = (apiType) => {
//     const segmentMap = {
//       'EQ': 'Equity',
//       'CE': 'Options',
//       'PE': 'Options',
...
//       'FUTCUR': 'F&O'
//     };
//     return segmentMap[apiType] || 'Equity';
//   };

//   // Handle script selection with auto-fill
//   const handleScriptSelect = (scrip) => {
//     // Map API data to your form fields
//     const mappedData = {
//       script: scrip.symbol,
//       scriptToken: scrip.token,
//       scriptName: scrip.name,
...
//     // If expiry exists in scrip data (not empty)
//     if (scrip.expiry && scrip.expiry.trim() !== '') {
//       mappedData.expiry = scrip.expiry;
//     } else {
//       mappedData.expiry = ''; // Clear if no expiry
//     }

//     // If strike price exists (not 0.000000)
//     if (scrip.strike && scrip.strike !== '0.000000') {
//       mappedData.strike_price = parseFloat(scrip.strike).toString();
//     } else {
//       mappedData.strike_price = ''; // Clear if no strike price
//     }
...
//     // Show success message with instrument details
//     toast.success(
//       <div>
//         <div className="font-bold">{scrip.symbol}</div>
//         <div className="text-xs">{scrip.name}</div>
//         <div className="text-xs mt-1">Type: {scrip.instrumenttype} | Token: {scrip.token}</div>
//       </div>,
//       {
//         position: "top-center",
//         autoClose: 3000,
...
//         <div
//           className={`w-full p-3 bg-white border rounded-lg cursor-pointer flex justify-between items-center transition-colors ${formData.script ? 'border-green-300 bg-green-50' : 'border-gray-300'} ${isScriptDropdownOpen ? 'border-blue-500 ring-1 ring-blue-500' : ''}`}
//           onClick={() => setIsScriptDropdownOpen(true)}
//         >
//           <div className="flex items-center gap-2 flex-1">
//             <FiSearch className="text-gray-400" />
//             <span className={formData.script ? 'text-gray-800 font-medium' : 'text-gray-400'}>
//               {formData.script || 'Search or select instrument...'}
//             </span>
//           </div>
//           <span className="text-gray-500">
...
//                   ref={searchInputRef}
//                   type="text"
//                   placeholder="Search by symbol, name, token or type..."
//                   value={searchQuery}
//                   onChange={(e) => setSearchQuery(e.target.value)}
//                   className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
//                 />
...
//                     // Get mapped values for display
//                     const mappedType = mapInstrumentType(scrip.instrumenttype);
//                     const mappedCategory = mapInstrumentCategory(scrip.instrumenttype);
//                     const mappedSegment = mapSegment(scrip.instrumenttype);
...
//                               {/* Strike Price Badge - Only if not zero */}
//                               {scrip.strike && scrip.strike !== '0.000000' && (
//                                 <span className="text-xs px-2 py-0.5 bg-green-100 text-green-800 rounded">
//                                   Strike: {parseFloat(scrip.strike).toFixed(2)}
//                                 </span>
//                               )}
//                             </div>
...
//               <span>{filteredScrips.length} instruments found</span>
//               <span className="text-blue-600">Select to auto-fill</span>
//             </div>
//           </div>
//         )}
//       </div>
...
//         </label>
//         <div
//           className={`w-full p-3 bg-white border rounded-lg flex justify-between items-center transition-colors ${value ? 'border-green-300 bg-green-50' : 'border-gray-300'} ${isOpen ? 'border-blue-500 ring-1 ring-blue-500' : ''} ${loading || disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:border-gray-400'}`}
//           onClick={() => !loading && !disabled && toggleDropdown(dropdownName)}
//         >
...
//                     value={formData.expiry}
//                     onChange={handleInputChange}
//                     placeholder="DDMMMYYYY (e.g., 30DEC2025)"
//                     className={`w-full p-3 border rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 ${formData.expiry ? 'border-green-300 bg-green-50 text-gray-800' : 'border-gray-300 bg-white text-gray-800'}`}
//                     required
//                   />
//                 </div>
//                 <div>
...
//                     className={`w-full p-3 border rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 ${formData.strike_price && formData.strike_price !== '' ? 'border-green-300 bg-green-50 text-gray-800' : 'border-gray-300 bg-white text-gray-800'}`}
//                     required
//                   />
//                 </div>
//               </div>
...
//                     onChange={handleInputChange}
//                     placeholder="e.g., 1 : 6.7"
//                     className="w-full p-3 bg-white border border-gray-300 rounded-lg text-gray-800 focus:outline-none focus:ring-1 focus:ring-blue-500"
//                     required
//                   />
//                 </div>
//               </div>
...
const CreateSignal = ({ data, onNext, onClose, onSignalCreated }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    position_status: '',
    subscriptionPlan: '',
    is_paid: false, // Default to free signal
  });


  console.log(formData,'formData...')
  // Signal type options for dropdown
  const signalTypeOptions = [
    { id: 'free', name: 'Free Signal', value: false },
    { id: 'paid', name: 'Paid Signal', value: true }
  ];
  ...
  const isFormValid = () => {
    return (
      ...
      formData.script &&
      formData.position_status &&
      formData.is_paid !== undefined // Added validation for is_paid
    );
  };
              ...
              <div className="mt-6 p-5 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-gray-800">Free Signal</span>
                          {formData.is_paid === false && (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-gray-800">Paid Signal</span>
                          {formData.is_paid === true && (
                ...
                {formData.is_paid !== undefined && (
                  <div className={`mt-3 p-2 rounded-md text-xs ${
                    ...
                    {formData.is_paid 
                      ? 'ℹ️ This will be a PAID signal. Users need an active subscription to view it.'
                      : 'ℹ️ This will be a FREE signal. All users can view it without any subscription.'}
                  </div>
                )}
              </div>

              {/* Display current is_paid status for debugging - remove in production */}
              <div className="text-xs text-gray-400 text-right">
                Status: {formData.is_paid ? 'Paid Signal' : 'Free Signal'}
              </div>
            </div>
