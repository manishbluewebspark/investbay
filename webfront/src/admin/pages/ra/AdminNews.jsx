...
// // pages/AdminNews.jsx
// import { useEffect, useState } from "react";
// import { FiPlus, FiEdit2, FiTrash2, FiEye, FiCalendar, FiClock, FiVideo, FiImage } from "react-icons/fi";
...
//           onClick={(e) => handleMediaClick(media.url, e)}
//         >
//           {/* Video Thumbnail - using first frame or video thumbnail if available */}
//           <video 
//             src={media.url} 
...
//   // Function to render media in preview modal
...
//             src={media.url} 
//             controls
//             className="rounded-lg w-full h-auto max-h-96 bg-black"
...
//             src={media.url} 
//             alt={`Media ${index + 1}`} 
//             className="rounded-lg w-full h-auto max-h-96 object-contain bg-gray-100"
//             onClick={() => window.open(media.url, '_blank')}
...
//                       {/* Media thumbnail - shows first image/video */}
//                       {item.images && item.images.length > 0 && (
//                         renderMediaThumbnail(item.images[0])
//                       )}
                      
//                       {/* Multiple media indicator */}
//                       {item.images && item.images.length > 1 && (
//                         <div className="relative -ml-8 mt-8 bg-gray-800 text-white text-xs px-1.5 py-0.5 rounded-full">
...
//                       {renderMediaPreview(selectedNews.images[0], 0)}
//                     </div>
//                   ) : (
//                     // Multiple media - grid layout
//                     <div className={`grid gap-4 ${
//                       selectedNews.images.length === 2 ? 'grid-cols-2' : 
//                       selectedNews.images.length === 3 ? 'grid-cols-3' : 
//                       'grid-cols-2 md:grid-cols-3'
//                     }`}>
//                       {selectedNews.images.map((media, idx) => (
//                         <div key={idx} className="relative">
//                           {renderMediaPreview(media, idx)}
//                         </div>
//                       ))}
//                     </div>
//                   )}
...
//               {/* Media type indicators in content */}
//               {selectedNews.images && selectedNews.images.length > 0 && (
...
//                         {isVideo ? <FiVideo size={12} /> : <FiImage size={12} />}
//                         {getFileExtension(media.url).toUpperCase()}
//                       </span>
//                     );
//                   })}
//                 </div>
...
const renderMediaThumbnail = (media) => {
  ...
  if (isVideo) {
    ...
    return (
      ...
      >
        ...
        onError={(e) => {
  e.target.onerror = null;
  e.target.alt = 'No Image'; // or '/no-image.png'
}}
        />
  ...
  // Fetch news stats - Memoize with useCallback
  ...
  // Fetch news with filters - Memoize with useCallback
  ...
  // Separate effect for filter changes (excluding initial mount)
  useEffect(() => {
  ...
  // Separate effect for stats refresh when needed
  useEffect(() => {
    if (isAdmin && userId) {
      fetchStats();
    }
  ...
  const handlePreview = useCallback((newsItem) => {
    setSelectedNews(newsItem);
    setShowPreview(true);
  }, []);
