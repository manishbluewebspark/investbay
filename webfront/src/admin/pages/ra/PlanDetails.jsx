...
const handleToggle = async () => {
    try {
        ...
        toast.success(`Plan is now ${newStatus.charAt(0).toUpperCase() + newStatus.slice(1)}`);
                ...
                <div className="md:col-span-2 flex flex-col gap-6">
                    <div className="bg-white rounded-2xl shadow-sm p-6">
                        ...
                        <div className="grid sm:grid-cols-2 gap-y-3 gap-x-6">
                            ...
                            <Detail label="Ideal Capital" value={`₹ ${plan.ideal_capital}`} />
                            <Detail label="Duration" value={plan.duration} />
                            <Detail label="Plan Price" value={`₹ ${plan.plan_price}`} />
                            <Detail label="Discount %" value={`${plan.discount} %`} />
                            <Detail label="Stoploss %" value={`${plan.stop_loss} %`} />
                            <Detail label="Avg Trades" value={plan.avg_trades} />
                        </div>

                        <div className="border-t border-gray-300 mt-5 pt-5">
                            <Detail label="Description" value={plan.short_description} />
                            <Detail label="Refund Policy" value={plan.refund_policy} />
                        </div>
                    </div>
                </div>
            </div>
