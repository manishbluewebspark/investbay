...
//     const emailCheckQuery = 'SELECT * FROM research_analysts WHERE email = $1';
...
//       // Optional: Mobile duplicate check
//       const mobileCheckQuery = 'SELECT * FROM research_analysts WHERE mobile = $1';
//       const mobileCheckResult = await client.query(mobileCheckQuery, [formattedMobile]);
      
//       if (mobileCheckResult.rows.length > 0) {
//         await client.query('ROLLBACK');
//         return res.status(409).json({
//           success: false,
...
//       // Optional: PAN duplicate check
//       const panCheckQuery = 'SELECT * FROM research_analysts WHERE pan = $1';
//       const panCheckResult = await client.query(panCheckQuery, [formattedPan]);
...
//       RETURNING id, name, email, user_id, role, status, profile_image, mobile, pan, created_at
//     `;
...
//       sebiNumber ? sebiNumber.trim() : null,
...
//       formattedMobile,  // ✅ Use formatted mobile (digits only)
//       formattedPan     // ✅ Use formatted PAN (validated)
//     ];
...
//         mobile: analyst.mobile,  // ✅ ADDED TO RESPONSE
//         pan: analyst.pan,        // ✅ ADDED TO RESPONSE
...
//       return res.status(409).json({
//         success: false,
//         message: "Duplicate entry detected. The email, mobile or PAN might already exist.",
//         error: error.message,
//       });
//     }
...
//       return res.status(400).json({
//         success: false,
//         message: "Input data too long. Please check mobile number (max 15 digits) and PAN (10 characters).",
